/**
 * b2c_user_event_history_service.js
 */
var b2c_user_event_history_service = {
	version : "",
	_default_params : {
		pageIndex : 1,
		pageUnit : 20
	...
	// [list] ////////////////////////////////////////////////////////
	,
	_b2c_user_event_history_list_template : [],
	...
	list : function(params, node_name, $target_template, is_append, uri) {
		...
		if(isEmpty(uri)) {
			uri = "/b2c_user_event_history/list/json";
		}
	...
	renderInList : function(jsonData, node_name, $target_template, is_append) {
		...
		var that = this;
		var b2c_user_event_history_list = {};
		if(isEmpty(node_name)) {
			node_name = "b2c_user_event_history_list";
		}

		if (jsonData && jsonData.result_code == "200") {
			b2c_user_event_history_list = jsonData;
			...
			if (list && Array.isArray(list)) {
				for (var j = 0; j < list.length; j++) {
					var b2c_user_event_history = list[j];

					// [Step-10] : To do action BEFORE displaying the template
					that.exeTodoBeforeRenderInList(b2c_user_event_history, j);
				}
			}
		}

		var $b2c_user_event_history_list = $('script[id=' + node_name + ']');

		if ($target_template) {
			$b2c_user_event_history_list = $target_template;
		}

		for (var i = 0; i < $b2c_user_event_history_list.length; i++) {
			var $target = $b2c_user_event_history_list.eq(i);

			if (!that._b2c_user_event_history_list_template[i]) {
				that._b2c_user_event_history_list_template.push($target.html());
			}
			fnRender(that._b2c_user_event_history_list_template[i], $target, b2c_user_event_history_list, is_append);
		}

		// [Step-20] : To do action AFTER displaying the template
		that.exeTodoAfterRenderInList(b2c_user_event_history_list);
	}
	...
	// [remove] ////////////////////////////////////////////////////////
	,
	remove : function(objData, uri, callback_success) {
		var that = this;

		var fnDeletecustomer_careTran = function() {
			...
			fnDoTransaction(uri, objData, function(jsonData) {
				...
				if (jsonData && typeof (jsonData) == "object") {
					if (jsonData.result_code == "200") {
						...
						that.renderInRemove(jsonData, callback_success);
					}
				}
			});
		};

		fnConfirm(fnDeletecustomer_careTran, "Are you sure to delete data?", "Confirm to delete");
	},
	renderInRemove : function(jsonData, callback_success) {
		var that = this;
		...
		}, "It is deleted successfully.", "Succeed to delete", null, "type-success", true);
	}
};
