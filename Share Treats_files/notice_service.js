...
var notice_service = {
	...
	renderInList : function(jsonData, node_name, $target_template, is_append) {
		...
		if(isEmpty(node_name)) {
			node_name = "notice_list";
		}

		if (jsonData && jsonData.result_code == "200") {
			notice_list = jsonData;
			...
			if (list && Array.isArray(list)) {
				for (var j = 0; j < list.length; j++) {
					...
					that.exeTodoBeforeRenderInList(notice, j);
				}
			}
		}

		var $notice_list = $('script[id=' + node_name + ']');

		if ($target_template) {
			$notice_list = $target_template;
		}

		for (var i = 0; i < $notice_list.length; i++) {
			var $target = $notice_list.eq(i);

			if (!that._notice_list_template[i]) {
				that._notice_list_template.push($target.html());
			}
			fnRender(that._notice_list_template[i], $target, notice_list, is_append);
		}

		// [Step-20] : To do action AFTER displaying the template
		that.exeTodoAfterRenderInList(notice_list);
	}
