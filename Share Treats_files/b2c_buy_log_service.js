/**
 * b2c_buy_log_service.js
 */
var b2c_buy_log_service = {
	version : "",
	_default_params : {
		pageIndex : 1,
		pageUnit : 20
	...
	exeTodoBeforeRenderInList : function(data, index) {
		...
		var movieFileNames = ["movieFileName"];
		fnChangeNasToCDN(movieFileNames, data);
	...
	renderInList : function(jsonData, node_name, $target_template, is_append) {
		...
		var that = this;
		var b2c_buy_log_list = {};
		if (isEmpty(node_name)) {
			node_name = "b2c_buy_log_list";
		}

		if (jsonData && jsonData.result_code == "200") {
			b2c_buy_log_list = jsonData;
			...
			if (list && Array.isArray(list)) {
				for (var j = 0; j < list.length; j++) {
					var b2c_buy_log = list[j];

					// [Step-10] : To do action BEFORE displaying the template
					that.exeTodoBeforeRenderInList(b2c_buy_log, j);
				}
			}
		}

		var $b2c_buy_log_list = $('script[id=' + node_name + ']');

		if ($target_template) {
			$b2c_buy_log_list = $target_template;
		}

		for (var i = 0; i < $b2c_buy_log_list.length; i++) {
			var $target = $b2c_buy_log_list.eq(i);

			if (!that._b2c_buy_log_list_template[i]) {
				that._b2c_buy_log_list_template.push($target.html());
			}
			fnRender(that._b2c_buy_log_list_template[i], $target, b2c_buy_log_list, is_append);
		}

		if (isEmpty(jsonData) || jsonData.result_code != "200") {
			b2c_buy_log_list = jsonData;
		}

		// [Step-20] : To do action AFTER displaying the template
		that.exeTodoAfterRenderInList(b2c_buy_log_list);
	}
